...
if __name__ == "__main__":
    ...
    # set up for training
    folder = 'train_data'
    model_dir = "../results/" + folder + "/saved_model/"
    suffix = '/train'
    ...
    for epoch in range(100):
        ...
        if epoch % 1 == 0:
            ...
            val = validate_model(model, tr_noaug_data_loader, criterion, epoch,
                                 prediction_folder='../results/train_data/', gpu_id=1)
            ...
            val = validate_model(model, ts_noaug_data_loader, criterion, epoch,
                                 prediction_folder='../results/test_data/', gpu_id=1)
            cvs_path = 'ep_' + str(epoch) + '_predictions.csv'
            arr = cvs_to_arr(cvs_file='../results/test_data/' + cvs_path)
            metrics = get_metrics(arr)
            ...
            val = validate_model(model, val_noaug_data_loader, criterion, epoch,
                                 prediction_folder='../results/val_data/', gpu_id=1)
            cvs_path = 'ep_' + str(epoch) + '_predictions.csv'
            arr = cvs_to_arr(cvs_file='../results/val_data/' + cvs_path)
            metrics = get_metrics(arr)
